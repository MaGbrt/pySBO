...
from eShotgun import eShotgun_utils
    

def par_eShotgun_run(DB, n_cycle, t_max, batch_size, id_run = None, comm = None):
    e = 0.1 
...
#        for i_cycle in range(n_cycle):
            ...
            with gpytorch.settings.max_cholesky_size(Global_Var.max_cholesky_size):
                ...
                X_next = eShotgun_utils.generate_batch(model = model,
                                                       f_lb = np.zeros(DB._dim), 
                                                       f_ub = np.ones(DB._dim), 
            ...
            # Update state
            print("Alg. eShotgun, cycle ", iter, " took --- %s seconds ---" % (M2_time - M1_time))
