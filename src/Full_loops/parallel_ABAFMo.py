...
from ABAFMo.Class_ABAFMo import ABAFMo
    
    

def par_ABAFMo_run(DB, n_cycle, t_max, batch_size, id_run = None, comm = None):
...
#        for i_cycle in range(n_cycle):
            ...
            M1_time = time()
            time_per_cycle[iter, 0] = 0
            print('Method status')
#            method.print_status()
            ######################################################################
            # Create a batch
            print('Generate batch')            
            X_next = method.generate_batch(iter)
            X_next = torch.tensor(X_next)
            print('Candidates are:\n', X_next)
            t_ap = time()
            time_per_cycle[iter, 1] = t_ap - t_start
            ...
            # Update state
            print('Update state')
            method.update_state(DB)

            print("Alg. ABAFMo, cycle ", iter, " took --- %s seconds ---" % (M2_time - M1_time))
