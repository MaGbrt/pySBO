...
# -*- coding: utf-8 -*-
"""
Created on Fri May 13 14:39:17 2022
...
def par_Turbo1_run(DB, n_cycle, t_max, batch_size, acq_f = "ei", id_run = None, comm = None, fit = 'custom'):
    ...
    if(my_rank == 0):
        ...
        iter = 0
        t_0 = time()
#        print('Start timer: 30 minutes from this point')
        t_current = 0.
...
#        for i_cycle in range(n_cycle):
            ...
            with gpytorch.settings.max_cholesky_size(Global_Var.max_cholesky_size):
                ...
                if (fit == 'custom'):
                    ...
                    raise NameError('Unknown fitting type')
                t_model = time ()
                time_per_cycle[iter, 0] = t_model - t_start
    #            print('Time to fit the model: ', time_per_cycle[iter, 0])

                ######################################################################
                # Create a batch
                # print('Generate batch')
                X_next = Turbo.generate_batch(mod = model, batch_size = batch_size, acqf = acq_f)
            ...
            print("Alg. TuRBO, cycle ", iter, " took --- %s seconds ---" % (t_end - t_start))
...
# TuRBO with fast fitting of the surrogate model
def par_fast_Turbo1_run(DB, n_cycle, t_max, batch_size, acq_f = "ei", id_run = None, comm = None):
    my_rank = comm.Get_rank()
    n_proc = comm.Get_size()
    if(my_rank == 0):
        target = np.zeros((1, n_cycle+1))
...
#                model.fit()
            ...
            if(Global_Var.sim_cost == -1): # real cost
                ...
                t_current += time_per_cycle[iter, 2] + Global_Var.sim_cost
            print("Alg. Fast TuRBO, cycle ", iter, " took --- %s seconds ---" % (t_end - t_start))
            print('Best known target is: ', torch.min(DB._y).numpy())
