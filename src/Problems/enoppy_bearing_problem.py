...
#------------------------------------------------------#
#------------- class enoppy_Bearing_problem -------------#
#------------------------------------------------------#


class Bearing(Single_Objective):
    """Class for the single-objective Multi-Product Batch Bearing problem.
    ...
    def __init__(self, n_dvar):
        assert n_dvar==10, 'The Multi-Product Batch Bearing problem is only define for 10 decision variables'
        self._enoppy_pb = ihaoavoa_2022.RollingElementBearingProblem()
    ...
    #-------------__str__-------------#
    def __str__(self):
        return "Multi-Product Batch Bearing problem "+str(self.n_dvar)+" decision variables "+str(self.n_obj)+" objective"
...
#        print('Evaluation done, returns: ', p, ' in ', end-start, ' s.')
    ...
    def get_bounds(self):
        ...
        :rtype: np.ndarray
        """
        if isinstance(self._enoppy_pb.bounds, np.ndarray):
            b = self._enoppy_pb.bounds.transpose()
        else:
            if isinstance(self._enoppy_pb.bounds, list):
                b = np.array(self._enoppy_pb.bounds).transpose()
            else:
                Warning('Not the right type of bounds')    
                b = self._enoppy_pb.bounds.transpose()
        return b
    ...
    #-------------plot-------------#
    def plot(self):
        print("[Bearing.py] Impossible to plot Bearing")
