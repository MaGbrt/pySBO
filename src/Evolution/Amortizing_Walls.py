...
#---------------------------------------------------------#
#-------------abstract class Amortizing_Walls-------------#
#---------------------------------------------------------#
class Amortizing_Walls(Positions_Updater):
    """Class for positions updater with amortizing walls."""
    ...
    def perform_positions_update(self, swarm):
        ...
        amortizing walls is used. If a decision variable gets out of the
        boundaries, it is set to the boundary value and the associated
        velocity component is multiplied by -r with r a random number 
        ...
        swarm.velocities[idx_dim_1, idx_dim_2]=-r*swarm.velocities[idx_dim_1, idx_dim_2]
        # upper bound
